...
def main() -> int:
    ...
    base = [v for k, v in sweeps.items() if k.startswith("baseline-")]
    if base:
        A("### Does the k-means predictor earn its labels?")
        A("")
        A("gpt2, first 256 MiB. `k=1` means one centroid, i.e. residuals against the")
        A("window mean -- scalar quantization with no learned predictor at all.")
        A("")
        A("| setting | k | label bytes | residual bytes | bits/val | ratio | max error |")
        A("|---|---|---|---|---|---|---|")
        for r in base:
    ...
    if tup:
        ...
        A("")
        A("| tuple size | k | label bits/val | bits/val | ratio |")
        A("|---|---|---|---|---|")
        for r in tup:
            lbits = 8.0 * r["label_bytes"] / max(1, r["stored_bytes"] / r["bits_per_value"] * 8)
            A(f"| {r['tuple_size']} | {ks(r)} | {lbits:.2f} | "
              f"{r['bits_per_value']:.2f} | {r['ratio_vs_source']:.2f}x |")
        A("")
    ...
    if vq:
        A("### The literal 'double k until max error fits' rule")
        A("")
        A(f"Running `--k-criterion vq` on gpt2 with `--max-k {vq['max_k']}`:")
        A("")
